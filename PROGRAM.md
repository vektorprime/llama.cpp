# autoresearch — Q4_K_M_CLONE quant size-reduction on Qwen3.5-0.8B
...
### Key function:
- `quantize_row_q4_K_M_CLONE_ref()` — reference quantize entry point
- `dequantize_row_q4_K_M_CLONE()` — dequantize entry point
...
20 min, it must be abandoned UNLESS it delivers ≥10% size reduction without
degrading KLD/same top p.

## Evaluation (LOCKED — never change these flags)

```bash
CUDA_VISIBLE_DEVICES=3 build/bin/llama-perplexity \
...
**Use `CUDA_VISIBLE_DEVICES=3`** (RTX 3050) for eval.
...
### Core quantization:
- `ggml/src/ggml-quants.c` — Q4_K_M_CLONE quantize/dequantize functions
- `ggml/src/ggml-quants.h` — declarations
- `ggml/src/ggml-common.h` — block struct `block_q4_K_M_CLONE`
- `ggml/src/ggml.c` — type traits registration
- `ggml/src/ggml-cpu/ggml-cpu.c` — CPU backend traits
- `ggml/src/ggml-cpu/ops.cpp` — CPU ops switch chains
- `ggml/src/ggml-cpu/repack.cpp` — repack logic
- `ggml/src/ggml-cuda/*` — CUDA backend dispatch

### llama layer:
- `src/llama-quant.cpp` — per-tensor type mixing
- `src/llama-model-loader.cpp` — ftype name/mapping
- `tools/quantize/quantize.cpp` — CLI quant name

### Locked (never touch):
- `tools/perplexity/**` — evaluation binary and source
- `/home/user/llm/models/Qwen3.5-0.8B/Qwen3.5-0.8B-Q4_K_M.gguf` — reference quant
...
- NEVER modify the Q4_K_M reference GGUF
