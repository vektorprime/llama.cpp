...
- **GGUF `gguf_set_arr_str` with empty strings preserves array count** (exp-039): Replacing 248K token strings with empty strings via `gguf_set_arr_str()` preserves n_vocab (derived from array count) while saving ~1.1 MB of string data. The GGUF overhead for empty string arrays is ~8 bytes per entry (offset table), so 248K empty strings take ~2 MB vs ~3.1 MB for full strings.
