...
5. **MSE-based local search optimization is near its ceiling (exp-052)**: Expanding the d/dmin/ls/lm local MSE search from exp-040's narrow ranges (±2% d, ±1 ls/lm, 2 phases) to aggressive ranges (±5% d 21-step, ±2 ls/lm, 6 phases with simulated annealing) REGRESSED quality: KLD 0.0624 vs 0.0586 (+6.3%), STP 86.64% vs 87.16% (-0.52pp). The MSE objective on FWHT-transformed weights is an imperfect proxy for end-to-end KLD. Larger search spaces introduce overfitting — the block-level MSE minimum does not translate to global perplexity improvement. Additionally, SA's random perturbation followed by deterministic fine-tuning can undo optimizations from earlier phases. The original 2-phase narrow search from exp-040 is near-optimal for the MSE→KLD transfer function. Future quality improvements must change the objective function (e.g., KLD-weighted importance) rather than expanding the search space on the same MSE proxy.

6. **Log-encoded d/dmin cannot replace fp16 — even with proven sc/m (exp-062)**: Replacing the 2+2 byte fp16 d/dmin with 1+1 byte uint8 log2 encoding (exp2((v-127)/16)) causes KLD 0.394 (625% above threshold) even when paired with proven 5+3 sc/m (exp-044's optimal config). This disproves the earlier hypothesis that exp-047's failure was driven by 3+3 sc/m alone. The d/dmin parameters define the quantization grid for all 256 weights in the superblock — going from fp16 (10-bit mantissa, 65536 levels) to 8-bit log (256 levels) loses irreplaceable information. The 256-level log grid is simply too coarse at typical d/dmin values (1e-4 to 1e-1). d/dmin are the single most quality-sensitive 2 bytes in the block — they MUST remain fp16.<｜end▁of▁thinking｜>
