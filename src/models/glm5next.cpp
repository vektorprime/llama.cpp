...
ggml_tensor * llama_model_glm5next::graph::build_dsa_layer(
    ...
    // Fix A: compact-gather sparse attention (long context, flash_attn)
    ...
    if (top_k && inp_kp && inp_kp->tail_cells && inp_kp->tail_mask) {
        ...
        const int64_t n_tps    = n_stream ? n_tokens / n_stream : 0;
        gather_B = n_tps * n_stream;
        // gate: flash_attn on, single-stream, long context (prefill and decode)
    ...
    if (top_k && use_gather) {
        // compact gather for B = n_tps * n_stream queries (B up to 512)
        // flatten batch: top_k [2048, B] -> [2048*B] 1D, tail [4, B] -> [4*B] 1D
        ggml_tensor * k_cache = inp_attn->mctx->get_k(ctx0, il); // [512, n_kv,1,1]
        const int64_t B = gather_B;
        const int64_t n_sel = 2048;
        const int64_t n_tail = hparams.indexer_kpool; // 4
        const int64_t n_pad = 2304 - (n_sel + n_tail); // 252
        // K_sel: [512, n_sel*B] F32
        ggml_tensor * top_k_flat = ggml_reshape_1d(ctx0, top_k, n_sel * B);
        ggml_tensor * K_sel_flat = ggml_get_rows(ctx0, k_cache, top_k_flat); // [512, n_sel*B,1,1] F32
        ggml_tensor * K_sel = ggml_reshape_4d(ctx0, K_sel_flat, k_cache->ne[0], n_sel, 1, B); // [512, n_sel,1,B]
        // K_tail: [512, n_tail*B] -> [512, n_tail,1,B]
        ggml_tensor * tail_flat = ggml_reshape_1d(ctx0, inp_kp->tail_cells, n_tail * B);
        ggml_tensor * K_tail_flat = ggml_get_rows(ctx0, k_cache, tail_flat); // [512, n_tail*B,1,1]
        ggml_tensor * K_tail = ggml_reshape_4d(ctx0, K_tail_flat, k_cache->ne[0], n_tail, 1, B);
        ggml_tensor * K_concat = ggml_concat(ctx0, K_sel, K_tail, 1); // [512, n_sel+n_tail,1,B] = [512,2052,1,B] F32
        ggml_tensor * K_pad = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, k_cache->ne[0], n_pad, 1, B);
        K_pad = ggml_scale(ctx0, K_pad, 0.0f);
        ggml_tensor * K_compact_f32 = ggml_concat(ctx0, K_concat, K_pad, 1); // [512,2304,1,B] F32
        ggml_tensor * K_compact = ggml_cast(ctx0, K_compact_f32, GGML_TYPE_F16);
        ggml_tensor * V_compact = K_compact;

        // mask: [2304,1,1,B] F16
        ggml_tensor * m_sel = ggml_new_tensor_4d(ctx0, GGML_TYPE_F16, n_sel, 1, 1, B);
        m_sel = ggml_fill(ctx0, m_sel, 0.0f);
        ggml_tensor * m_tail = ggml_reshape_4d(ctx0, inp_kp->tail_mask, n_tail, 1, 1, B); // tail_mask [4,B] -> [4,1,1,B]
        ggml_tensor * m_pad = ggml_new_tensor_4d(ctx0, GGML_TYPE_F16, n_pad, 1, 1, B);
        m_pad = ggml_fill(ctx0, m_pad, -INFINITY);
        ggml_tensor * mask_tmp = ggml_concat(ctx0, m_sel, m_tail, 0); // [2052,1,1,B]
        ggml_tensor * mask = ggml_concat(ctx0, mask_tmp, m_pad, 0); // [2304,1,1,B] F16
