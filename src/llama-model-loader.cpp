...
llama_model_loader::llama_model_loader(
    ...
    if (!fname.empty()) {
        ...
        if (has_q8_outlier_metadata()) {
            ...
            for (const auto & t : q8_meta) {
                const ggml_tensor * base = get_tensor_meta(t.base_name.c_str());
                if (!base) {
                    throw std::runtime_error(format("q8 outlier: base tensor '%s' not found", t.base_name.c_str()));
                }
                if (base->type != GGML_TYPE_Q8_0) {
                    LLAMA_LOG_ERROR("q8 outlier: base tensor '%s' has type %s, expected Q8_0\n",
                        t.base_name.c_str(), ggml_type_name(base->type));
                    throw std::runtime_error(format("q8 outlier: base tensor '%s' has wrong type %s, expected Q8_0",
                        t.base_name.c_str(), ggml_type_name(base->type)));
                }

                const ggml_tensor * idx = get_tensor_meta(t.idx_name.c_str());
                if (!idx) {
                    throw std::runtime_error(format("q8 outlier: index tensor '%s' not found", t.idx_name.c_str()));
                }
                if (idx->type != GGML_TYPE_I32) {
                    LLAMA_LOG_ERROR("q8 outlier: index tensor '%s' has type %s, expected I32\n",
                        t.idx_name.c_str(), ggml_type_name(idx->type));
                    throw std::runtime_error(format("q8 outlier: index tensor '%s' has wrong type %s, expected I32",
                        t.idx_name.c_str(), ggml_type_name(idx->type)));
                }
        ...
        if (has_q4_outlier_metadata()) {
            auto q4_meta = read_q4_outlier_metadata();
            for (const auto & t : q4_meta) {
                const ggml_tensor * base = get_tensor_meta(t.base_name.c_str());
                if (!base) {
                    throw std::runtime_error(format("q4 outlier: base tensor '%s' not found", t.base_name.c_str()));
                }
                if (base->type != GGML_TYPE_Q4_0) {
                    LLAMA_LOG_ERROR("q4 outlier: base tensor '%s' has type %s, expected Q4_0\n",
                        t.base_name.c_str(), ggml_type_name(base->type));
                    throw std::runtime_error(format("q4 outlier: base tensor '%s' has wrong type %s, expected Q4_0",
                        t.base_name.c_str(), ggml_type_name(base->type)));
                }

                const ggml_tensor * idx = get_tensor_meta(t.idx_name.c_str());
                if (!idx) {
                    throw std::runtime_error(format("q4 outlier: index tensor '%s' not found", t.idx_name.c_str()));
                }
                if (idx->type != GGML_TYPE_I32) {
                    LLAMA_LOG_ERROR("q4 outlier: index tensor '%s' has type %s, expected I32\n",
                        t.idx_name.c_str(), ggml_type_name(idx->type));
                    throw std::runtime_error(format("q4 outlier: index tensor '%s' has wrong type %s, expected I32",
                        t.idx_name.c_str(), ggml_type_name(idx->type)));
                }
                check_tensor_dims(t.idx_name, {2, (int64_t)t.n_blocks}, true);
                ...
                if (vals->type != GGML_TYPE_BF16 && vals->type != GGML_TYPE_Q8_0 && vals->type != GGML_TYPE_Q4_0) {
                    LLAMA_LOG_ERROR("q4 outlier: values tensor '%s' has type %s, expected BF16, Q8_0, or Q4_0\n",
                        t.values_name.c_str(), ggml_type_name(vals->type));
                    throw std::runtime_error(format("q4 outlier: values tensor '%s' has wrong type %s, expected BF16, Q8_0, or Q4_0",
                        t.values_name.c_str(), ggml_type_name(vals->type)));
                }
                if (vals->type == GGML_TYPE_BF16) {
                    check_tensor_dims(t.values_name, {32, (int64_t)t.n_blocks}, true);
                } else if (vals->type == GGML_TYPE_Q8_0) {
                    check_tensor_dims(t.values_name, {32, (int64_t)t.n_blocks}, true);
                } else {
                    // Q4_0: same dimensions [32, n_blocks]
                    check_tensor_dims(t.values_name, {32, (int64_t)t.n_blocks}, true);
                }
            }
        }
...
bool llama_model_loader::has_q8_outlier_metadata() const {
    const int kid = gguf_find_key(metadata, "llama.q8_outlier.version");
    return kid >= 0;
}

std::vector<llama_model_loader::llama_q8_outlier_tensor_meta> llama_model_loader::read_q8_outlier_metadata() {
    std::vector<llama_q8_outlier_tensor_meta> result;

    uint32_t tensor_count = 0;
    if (!get_key("llama.q8_outlier.tensor_count", tensor_count, false)) {
        return result;
    }
    ...
    for (uint32_t i = 0; i < tensor_count; i++) {
        std::string prefix = format("llama.q8_outlier.tensor.%d.", i);
...
bool llama_model_loader::has_q4_outlier_metadata() const {
    const int kid = gguf_find_key(metadata, "llama.q4_outlier.version");
    return kid >= 0;
}

std::vector<llama_model_loader::llama_q4_outlier_tensor_meta> llama_model_loader::read_q4_outlier_metadata() {
    std::vector<llama_q4_outlier_tensor_meta> result;

    uint32_t tensor_count = 0;
    if (!get_key("llama.q4_outlier.tensor_count", tensor_count, false)) {
        return result;
    }
...
bool llama_model_loader::load_all_data(
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (use_mmap) {
            ...
            // DEBUG: check tensor data pointer before upload
            {
