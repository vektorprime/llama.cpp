...
static void ggml_cuda_op_mul_mat_outlier_blocks_impl(
    ...
    if (ggml_custom_logs_enabled()) {
        fprintf(stderr, "[delta-cuda] kernel launch: grid=(%u,%u,%u) block=(%u,%u,%u) values_type=%s\n",
                ...
                (value_type_i32 == 4 ? "I8_single" : "I8") : "BF16");
        fflush(stderr);
    }
...
static void ggml_cuda_op_mul_mat_outlier_blocks_merged_impl(
    ...
    if (ggml_custom_logs_enabled()) {
        fprintf(stderr, "[delta-cuda-merged] kernel launch: n_merged_runs=%lld n_tokens=%lld values_type=%s\n",
                ...
                values_type == GGML_TYPE_Q8_0 ? "Q8_0" : values_type == GGML_TYPE_I8 ? "I8" : "BF16");
        fflush(stderr);
    }
...
void ggml_cuda_op_mul_mat_outlier_fused(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (sanity_count < 3) {
        ...
        fprintf(stderr, "[fused-sanity] %s ne=[%lld,%lld,%lld,%lld] nb=[%zu,%zu,%zu,%zu] n_tokens=%lld\n",
            w->name ? w->name : "?",
        ...
        if (ggml_custom_logs_enabled() && w->type == GGML_TYPE_Q4_0 && n_rows_out > 0 && n_blocks_per_row > 0) {
            ...
            // Read CSR to apply deltas in CPU reference too
