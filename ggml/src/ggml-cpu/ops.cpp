...
void ggml_compute_forward_mul_mat_outlier_blocks(
    ...
    if (ith == 0 && n_tokens > 0 && ggml_custom_logs_enabled()) {
        ...
        if (_last_outlier_name == nullptr || strcmp(_last_outlier_name, tname) != 0) {
            _last_outlier_name = tname;
            // Compute L2 norm of the correction output
            double l2 = 0.0;
            int64_t nnz = 0;
            for (int64_t i = 0; i < n_rows_out * n_tokens; i++) {
                if (dst_data[i] != 0.0f) {
                    l2 += (double)dst_data[i] * dst_data[i];
                    nnz++;
                }
            }
            fprintf(stderr, "[outlier-corr] CPU: %s — correction L2=%.6f, nnz=%lld/%lld (%.1f%%), blocks=%lld\n",
                tname, sqrt(l2), (long long)nnz, (long long)(n_rows_out * n_tokens),
                100.0 * nnz / (n_rows_out * n_tokens), (long long)n_blocks);
            fflush(stderr);
        }
    }
}

// ggml_compute_forward_mul_mat_outlier_blocks_merged

void ggml_compute_forward_mul_mat_outlier_blocks_merged(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    const int32_t * merged_idx_data = (const int32_t *) merged_idx->data;
    const float * x_data            = (const float *) x->data;
    const int64_t x_stride          = x->nb[1] / sizeof(float);
    ...
    for (int64_t ir = 0; ir < n_merged_runs; ir++) {
        const int32_t row            = merged_idx_data[ir * 4];
        const int32_t start_block_col = merged_idx_data[ir * 4 + 1];
        const int32_t count          = merged_idx_data[ir * 4 + 2];
        const int32_t values_start   = merged_idx_data[ir * 4 + 3];

        if (row < (int32_t)row0 || row >= (int32_t)row1) {
            continue;
        }

        const int64_t col0 = (int64_t)start_block_col * 32;
        const int64_t total_elems = (int64_t)count * 32;
        if (col0 + total_elems - 1 >= n_cols) {
            continue;
        }
...
void ggml_compute_forward_mul_mat_outlier_fused(
    ...
    GGML_ASSERT(dst->ne[1] == n_tokens);
    GGML_ASSERT(x->ne[0] == n_cols);
    GGML_ASSERT(w->ne[0] == n_cols);
    GGML_ASSERT(w->ne[1] == n_rows_out);
    ...
    // Zero the output rows for this thread
    for (int64_t row = row0; row < row1; row++) {
        for (int64_t it = 0; it < n_tokens; it++) {
            dst_data[row + it * dst_stride] = 0.0f;
        }
    ...
    const bool is_q8_0_weight = (w->type == GGML_TYPE_Q8_0);
    const bool is_q4_0_weight = (w->type == GGML_TYPE_Q4_0);
    const bool is_q2_k_weight = (w->type == GGML_TYPE_Q2_K);

    const block_q8_0 * w_q8 = (const block_q8_0 *) w->data;
    ...
    std::vector<std::vector<std::pair<int32_t, int32_t>>> row_deltas(n_rows_out);
    for (int64_t k = 0; k < n_blocks; k++) {
        int32_t r = idx_data[k * 2];
        int32_t bc = idx_data[k * 2 + 1];
        if (r >= 0 && r < (int32_t)n_rows_out && bc >= 0 && bc < (int32_t)n_blocks_per_row) {
            row_deltas[r].push_back({bc, (int32_t)k});
        }
    }

    for (int64_t row = row0; row < row1; row++) {
        const auto & deltas = row_deltas[row];
        // Build a quick lookup: block_col -> delta_block_idx
        // Since deltas are typically very sparse, linear scan is fine
        // But we can sort for potential binary search
        // For now, just linear scan since n_deltas_per_row is small
        ...
        for (int64_t it = 0; it < n_tokens; it++) {
            ...
            for (int64_t bk = 0; bk < n_blocks_per_row; bk++) {
                ...
                // Check if this block has a delta
                int32_t delta_idx = -1;
                for (const auto & [dbc, didx] : deltas) {
                    if (dbc == (int32_t)bk) {
                        delta_idx = didx;
                        break;
                    }
                ...
                for (int64_t j = 0; j < 32; j++) {
                    ...
                    float w_val;
                    if (is_q4_0_weight) {
                        const int q = (w_q4[weight_idx].qs[j / 2] >> (4 * (j % 2))) & 0xF;
                        w_val = GGML_FP16_TO_FP32(w_q4[weight_idx].d) * (q - 8);
                    } else if (is_q8_0_weight) {
                        w_val = GGML_FP16_TO_FP32(w_q8[weight_idx].d) * w_q8[weight_idx].qs[j];
                    } else {
                    ...
                    if (has_delta) {
                        if (is_single_val) {
                            ...
                            w_val += GGML_BF16_TO_FP32(values_bf16[delta_idx * 32 + j]);
                        }
                    }
