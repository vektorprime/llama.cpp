...
static void quantize_row_iq2_xxs_impl(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t n, const float * GGML_RESTRICT quant_weights) {
    ...
    for (int ibl = 0; ibl < nbl; ++ibl) {
        ...
        for (int ib = 0; ib < QK_K/32; ++ib) {
            ...
            // exp-046: greedy per-element level perturbation — pick the single most
            // impactful dimension per 8D chunk (highest weight × |xval|), try ±1.
            // The uniform id shift in scale search moves all 32 levels together;
            // individual flips explore combinations it cannot reach.
            if (scale > 0) {
                for (int k = 0; k < 4; ++k) {
                    int8_t * ch = L + 8*k;
                    int8_t save[8]; memcpy(save, ch, 8);
                    float cur = 0, best_err = 0;
                    for (int i = 0; i < 8; ++i) cur += weight[8*k+i]*fabsf(scale*(2*ch[i]+1) - xval[8*k+i]);
                    best_err = cur;
                    int8_t best_L[8]; memcpy(best_L, save, 8);
                    int best_dim = 0; float best_w = 0;
                    for (int i = 0; i < 8; ++i) {
                        float wv = weight[8*k+i]*fabsf(xval[8*k+i]);
                        if (wv > best_w) { best_w = wv; best_dim = i; }
                    }
                    for (int dir = -1; dir <= 1; dir += 2) {
                        int nl = save[best_dim] + dir;
                        if (nl < 0 || nl > 3) continue;
                        uint16_t u = 0;
                        for (int j = 0; j < 8; ++j) u |= ((j == best_dim ? nl : save[j]) << 2*j);
                        if (u >= 43692) continue;
                        int g = kmap_q2xs[u];
                        if (g >= 0) {
