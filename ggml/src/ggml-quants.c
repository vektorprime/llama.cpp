...
void quantize_row_q4_K_impl(const float * GGML_RESTRICT x, block_q4_K * GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
    assert(n_per_row % QK_K == 0);
...
void quantize_row_q4_K_M_CLONE_ref(const float * GGML_RESTRICT x, block_q4_K_M_CLONE * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        float inv_min   = max_min   > 0 ? 7.f/max_min   : 0.f;
        ...
        float best_dmin_val = max_min/7.f;
        ...
        // Pack scales with best ls/lm values (5+3 bits per byte)
        for (int j = 0; j < QK_K/32; ++j) {
            y[i].scales[j] = (best_ls[j] & 0x1F) | ((best_lm[j] & 0x7) << 5);
        }
        ...
        for (int j = 0; j < QK_K/32; ++j) {
            uint8_t sc = y[i].scales[j] & 0x1F;
            uint8_t m  = (y[i].scales[j] >> 5) & 0x7;
            const float d = GGML_FP16_TO_FP32(y[i].d) * sc;
            if (!d) continue;
            const float dm = GGML_FP16_TO_FP32(y[i].dmin) * m;
            for (int ii = 0; ii < 32; ++ii) {
...
void dequantize_row_q4_K_M_CLONE(const block_q4_K_M_CLONE * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t b = 0; b < nb; b++) {
        const float dall = GGML_FP16_TO_FP32(x[b].d);
        const float dmin = GGML_FP16_TO_FP32(x[b].dmin);
        const uint8_t * q = x[b].qs;
        float * yy = y + b * QK_K;

        for (int j = 0; j < QK_K/32; j += 2) {
            uint8_t sc0 = x[b].scales[j] & 0x1F;
            uint8_t m0  = (x[b].scales[j] >> 5) & 0x7;
            uint8_t sc1 = x[b].scales[j+1] & 0x1F;
            uint8_t m1  = (x[b].scales[j+1] >> 5) & 0x7;
    ...
    if (!quant_weights) {
        ...
        for (int64_t row = 0; row < nrow; row++) {
            ...
            for (int i = 0; i < nb; i++) {
                ...
                float m_block = make_qp_quants(QK_K/32, 7, mins,   Lm, sw);
                for (int j = 0; j < QK_K/32; ++j) {
                    y[i].scales[j] = (Ls[j] & 0x1F) | ((Lm[j] & 0x7) << 5);
                }
                y[i].d = GGML_FP32_TO_FP16(d_block);
                y[i].dmin = GGML_FP32_TO_FP16(m_block);
    ...
    for (int64_t row = 0; row < nrow; ++row) {
        ...
        for (int ibl = 0; ibl < nblock; ++ibl) {
            ...
            quantize_row_iq4_nl_impl(QK_K, 32, src + QK_K*ibl, &iq4[ibl].d, iq4[ibl].qs, &iq4[ibl].scales_h, iq4[ibl].scales_l,
                    scales, weight, L, kvalues_iq4nl, qw, 8);
        }
        src += n_per_row;
