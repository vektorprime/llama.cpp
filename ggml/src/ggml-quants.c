...
void iq2xxs_learn_grid(const float * GGML_RESTRICT x, const float * GGML_RESTRICT weights,
    ...
    for (int trial = 0; trial < num_trials; ++trial) {
        ...
        /* --- Density-aware centroid splitting (LBG VQ split) ---
         * After K-means, identify over-utilized centroids and split them into
         * under-utilized slots. This reallocates codebook capacity to match data density. */
        {
            int * counts = (int *)calloc(grid_size, sizeof(int));
            GGML_ASSERT(counts);
            int64_t avg_cnt = n_samples / grid_size;
            if (avg_cnt < 1) avg_cnt = 1;
            for (int sr = 0; sr < 3; ++sr) {
                memset(counts, 0, grid_size * sizeof(int));
                for (int64_t s = 0; s < n_samples; ++s) {
                    int k = assignments[s];
                    if (k >= 0 && k < grid_size) counts[k]++;
                }
                int max_k = 0, min_k = 0;
                int max_cnt = 0, min_cnt = n_samples + 1;
                for (int k = 0; k < grid_size; ++k) {
                    if (counts[k] > max_cnt) { max_cnt = counts[k]; max_k = k; }
                    if (counts[k] < min_cnt) { min_cnt = counts[k]; min_k = k; }
                }
                if (max_cnt <= 2 * avg_cnt) break;

                /* Clone centroid max_k into min_k with small perturbation */
                int sd = (max_k + sr * 3) % 8;
                for (int i = 0; i < 8; ++i) {
                    float v = centroids_float[8*max_k + i];
                    if (i == sd) v += 1.0f;
                    if (v > 127.0f) v = 127.0f;
                    if (v < 0.0f) v = 0.0f;
                    centroids_float[8*min_k + i] = v;
                }

                /* Re-run 5 K-means iterations to settle split */
