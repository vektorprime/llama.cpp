...
static void usage(const char * executable) {
    ...
    printf("  --outlier-value-type bf16|q8_0|q4_0\n");
...
int llama_quantize(int argc, char ** argv) {
    ...
    for (; arg_idx < argc && strncmp(argv[arg_idx], "--", 2) == 0; arg_idx++) {
        if (strcmp(argv[arg_idx], "--leave-output-tensor") == 0) {
            ...
            if (arg_idx < argc-1 && striequals(argv[arg_idx + 1], "q8_0")) {
            ...
            } else if (arg_idx < argc-1 && striequals(argv[arg_idx + 1], "q4_0")) {
                ++arg_idx;
                ...
                fprintf(stderr, "%s: --outlier-value-type must be 'bf16', 'q8_0', or 'q4_0'\n", __func__);
                usage(argv[0]);
            }
